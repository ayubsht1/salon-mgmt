...
export default function Home() {
  ...
  async function handleAuth(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (authMode === "register") {
        ...
        const result = await api<{ data: { access: string; user: User } }>(
          "/login/",
          { method: "POST", body: JSON.stringify(values) },
        );
        setLocalToken(result.data.access);
        setLocalUser(result.data.user);
        setSignedOut(false);
        localStorage.setItem("salon_access", result.data.access);
        localStorage.setItem("salon_user", JSON.stringify(result.data.user));
        setAuthOpen(false);
      }
    } catch (error) {
      setMessage(
  ...
  async function bookAppointment(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!selectedService || !user || !token) return;
    setBookingBusy(true);
    setMessage("");
    const appointment_datetime = new FormData(event.currentTarget).get(
    ...
    try {
      ...
      setMessage("Your appointment request is in. We will confirm it shortly.");
  ...
  async function createService(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setServices((current) =>
        [...current, service].sort((left, right) =>
    ...
    <main>
      <nav className="nav shell">
        ...
        <div className="nav-actions">
          ...
          <a className="text-button" href="/appointments">
            Appointments
          </a>
          {user ? (
            <>
      ...
      <section className="hero shell" id="top">
        <div className="hero-copy">
          ...
          <p className="hero-text">
            Thoughtful cuts, color, and care in a calm little studio made for
        ...
        <div className="hero-art">
          ...
          <div className="hero-card">
            ...
            <small>Tuesday to Saturday</small>
          </div>
        </div>
      </section>
      {message && (
      ...
      <section className="section shell" id="services">
        <div className="section-heading">
          ...
          <p>
            Every appointment begins with a conversation and ends with you
            feeling a little more at home in your skin.
          </p>
        </div>
        {loading ? (
          <div className="loading">Loading our services...</div>
        ) : services.length ? (
          <div className="service-grid">
            {services.map((service, index) => (
              <article className="service-card" key={service.id}>
                ...
                <p>
                  {service.description ||
                    "A considered service, tailored to you."}
                </p>
                ...
                <button
                  className="book-link"
                  onClick={() =>
                    user ? setSelectedService(service) : setAuthOpen(true)
                  }
                >
      ...
      {user && (
        <section className="appointments shell">
          ...
          {appointments.length ? (
            ...
            <p className="muted">Your upcoming visits will appear here.</p>
          )}
        </section>
      )}
      ...
      {selectedService && (
        <div
          ...
          onMouseDown={(event) =>
            event.target === event.currentTarget && setSelectedService(null)
          }
        >
          <form className="modal" onSubmit={bookAppointment}>
            <button
              type="button"
            ...
            <button className="primary-button full" disabled={bookingBusy}>
      ...
      {serviceOpen && (
        <div
          className="modal-backdrop"
          onMouseDown={(event) =>
            event.target === event.currentTarget && setServiceOpen(false)
          }
        >
          <form className="modal" onSubmit={createService}>
            ...
            <h2>Add a service</h2>
            <Field label="Service name" name="name" required maxLength={150} />
            <label className="field">
            ...
            <button className="primary-button full" disabled={serviceBusy}>
      ...
      {authOpen && (
        ...
        >
          <form className="modal" onSubmit={handleAuth}>
            <button
              type="button"
              className="close-button"
            ...
            <p className="eyebrow">Welcome to Serein</p>
            <h2>
              {authMode === "login" ? "Good to see you" : "Make a little room"}
            </h2>
            {authMode === "register" && (
              <div className="two-fields">
